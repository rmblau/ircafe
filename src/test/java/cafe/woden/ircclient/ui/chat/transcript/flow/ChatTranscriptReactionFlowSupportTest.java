...
class ChatTranscriptReactionFlowSupportTest {
  ...
  void setReactionChipActionHandlerRebindsExistingStates() {
    ChatTranscriptReactionSummarySupport reactionSummarySupport = mock(ChatTranscriptReactionSummarySupport.class);
    ...
    ChatTranscriptReactionFlowSupport.Context context =
        new ChatTranscriptReactionFlowSupport.Context(docs, states, r -> {}, reactionSummarySupport);

    ChatTranscriptStore.ReactionChipActionHandler handler = (target, messageId, reactionToken, unreactRequested) -> {};
    support.setReactionChipActionHandler(context, handler);

    verify(reactionSummarySupport).setReactionChipActionHandler(handler, org.mockito.ArgumentMatchers.anyMap());
  }
  ...
  void applyAndRemoveEnsureTargetBeforeDelegating() {
    ...
    verify(reactionSummarySupport)
        .applyMessageReaction(ref, docs.get(ref), states.get(ref).reactionSummary, "m-1", ":)", "alice", 10L);
    verify(reactionSummarySupport)
        .removeMessageReaction(ref, docs.get(ref), states.get(ref).reactionSummary, "m-1", ":)", "alice", 11L);
  ...
  void missingStateReturnsFalseForReactionLookup() {
    ...
    ChatTranscriptReactionSummarySupport reactionSummarySupport = mock(ChatTranscriptReactionSummarySupport.class);
    ChatTranscriptReactionFlowSupport.Context context =
        new ChatTranscriptReactionFlowSupport.Context(Map.of(), Map.of(), r -> {}, reactionSummarySupport);

    assertFalse(support.hasReactionFromNick(context, new TargetRef("srv", "#chan"), "m-1", ":)", "alice"));
  }
