...
public final class Ircv3ChatHistoryCommandBuilder {
  ...
  public static String buildLatest(String target, String selectorOrWildcard, int limit) {
    String t = sanitizeTarget(target);
  ...
  public static String buildAround(String target, String selector, int limit) {
    String t = sanitizeTarget(target);
    String s = sanitizeSelector(selector);
    int lim = clampLimit(limit);
  ...
  public static String buildBetween(
  ...
  /** Returns a normalized request selector, or an empty string when the input is invalid. */
  public static String normalizeRequestSelectorOrEmpty(String selector) {
    String raw = Objects.toString(selector, "").trim();
    if (raw.isEmpty()) return "";
    try {
      String normalized = sanitizeSelector(raw);
      int eq = normalized.indexOf('=');
      String key = eq > 0 ? normalized.substring(0, eq) : "";
  ...
  /** Returns a normalized selector or wildcard, or an empty string when invalid. */
  public static String normalizeRequestSelectorOrWildcardOrEmpty(String selector) {
    String raw = Objects.toString(selector, "").trim();
    if ("*".equals(raw)) return "*";
    return normalizeRequestSelectorOrEmpty(raw);
  }
  ...
  public static String sanitizeSelector(String selector) {
  ...
  public static String sanitizeSelectorOrWildcard(String selector) {
    String s = Objects.requireNonNull(selector, "selector").trim();
    if (s.equals("*")) return "*";
