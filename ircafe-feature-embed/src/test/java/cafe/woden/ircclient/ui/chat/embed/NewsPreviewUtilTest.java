...
class NewsPreviewUtilTest {
  ...
  void parseArticleDocumentExtractsMetadataAndExtendedSummary() {
    ...
    LinkPreview preview =
        NewsPreviewUtil.parseArticleDocument(
            doc,
            "https://abcnews.com/GMA/Culture/robert-duvall-star-godfather-apocalypse-now-dead-95/story?id=96490943");
  ...
  void heuristicsRecognizeNewsUrlAndStructuredDescription() {
    assertTrue(
        NewsPreviewUtil.isLikelyNewsArticleUrl(
            "https://abcnews.com/GMA/Culture/robert-duvall-star-godfather-apocalypse-now-dead-95/story?id=96490943"));
    assertTrue(
        NewsPreviewUtil.looksLikeNewsDescription(
  ...
  void parseArticleDocumentSupportsRequestedPublishers() {
    ...
    for (Case c : cases) {
      ...
      assertTrue(
          preview.description().contains("Publisher: " + c.expectedPublisher()),
          preview.description());
      assertTrue(preview.description().contains("Summary:\n"), preview.description());
  ...
  void parseArticleDocumentReflowsSummaryIntoNaturalParagraphs() {
    ...
    LinkPreview preview =
        NewsPreviewUtil.parseArticleDocument(
            doc, "https://www.reuters.com/world/europe/sample-article-2026-02-16/");
    ...
    // The summary should be grouped into compact paragraphs, not one blank line between every
    // sentence.
