...
final class Ircv3ScramSaslExchange {
  ...
  Ircv3ScramSaslExchange(String digest, String user, String pass) {
    this.digest = digest;
  ...
  void onServerFirst(String serverFirst) throws Ircv3SaslException {
    ...
    if (nonce == null || saltB64 == null || iterationsRaw == null) {
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED, "Invalid SCRAM server-first-message");
    }
    if (!nonce.startsWith(clientNonce)) {
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED,
    ...
    try {
      ...
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED, "Invalid SCRAM salt (base64)");
    }
    ...
    try {
      ...
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED, "Invalid SCRAM iteration count");
    }
    if (iterations <= 0) {
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED, "Invalid SCRAM iteration count");
  ...
  void onServerFinal(String serverFinal) throws Ircv3SaslException {
    Map<String, String> kv = parseScramKvs(Objects.toString(serverFinal, ""));
    String err = kv.get("e");
    if (err != null && !err.isEmpty()) {
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED, "SCRAM error from server: " + err);
    }
    String verifier = kv.get("v");
    if (verifier == null) {
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED, "Missing SCRAM server signature");
    }
    if (!Objects.equals(serverSignatureB64, verifier)) {
      throw new Ircv3SaslException(
          Ircv3SaslException.Reason.SASL_FAILED, "SCRAM server signature verification failed");
    }
  ...
  private static byte[] hmac(byte[] key, byte[] msg, String digest) throws Ircv3SaslException {
    try {
      ...
      throw new Ircv3SaslException(Ircv3SaslException.Reason.OTHER, "HMAC failure", e);
    }
  }

  private static byte[] hash(byte[] in, String digest) throws Ircv3SaslException {
    try {
      return java.security.MessageDigest.getInstance(digest).digest(in);
    } catch (Exception e) {
      throw new Ircv3SaslException(Ircv3SaslException.Reason.OTHER, "Digest failure", e);
    }
  }
