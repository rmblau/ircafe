...
public final class Ircv3ChghostExtensionProvider
  ...
  public Set<Ircv3InboundCommandOperation> inboundCommandOperations() {
    return Set.of(Ircv3InboundCommandOperation.CHGHOST);
  }

  @Override
  public List<Ircv3InboundCommandSignal> parse(
      Ircv3InboundCommandOperation operation, Ircv3InboundCommandRequest request) {
    if (operation != Ircv3InboundCommandOperation.CHGHOST || request == null) {
      return List.of();
    }

    return Ircv3ChghostParser.parse(request.command(), request.parameters())
        .map(
            change ->
                List.<Ircv3InboundCommandSignal>of(
                    new Ircv3InboundCommandSignal.HostChangedObserved(
                        ...
                        change.user(),
                        change.host(),
                        change.hostmaskFor(request.sourceNick()).orElse(""))))
        .orElseGet(List::of);
  }
