 ...
 * {@code META-INF/services/cafe.woden.ircclient.notify.spi.CustomSoundPlaybackProvider}. IRCafe
 * owns executor scheduling, playback freshness/rate limiting, provider failure isolation, Java
 * Sound fallback, and the lifecycle of the supplied file.
 */
public interface CustomSoundPlaybackProvider {
   ...
   *
   * <p>Return {@code true} only after taking ownership of this playback request. Return {@code
   * false} for unsupported or deliberately unhandled files so IRCafe can try later providers and
   * its Java Sound fallback. Throwing an exception does not claim the request; IRCafe isolates the
   * failure and continues the chain. Providers must not move, delete, or retain ownership of the
   * supplied path.
   *
